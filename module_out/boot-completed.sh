...
# Start sulog mirror script in background
sh $(dirname $0)/sulog_mirror.sh &
...
exec /system/bin/app_process -Djava.class.path=$(dirname $0)/daemon.dex / --nice-name=SuToaster com.suisho.kernelsugranttoast.Entry "$customToastText" "$ignoredPackages" "$packageSearchDepth" "$autoDeleteLog"
